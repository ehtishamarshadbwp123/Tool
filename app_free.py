...
# --- Feature 2: Symbol Rate & Spectrum Section ---
    ...
    # Restrictions and help tooltips removed
    bw = st.number_input("Bandwidth (MHz)", value=36.0, step=0.1)
    ...
    # --- Feature 3: KU-Band LO Calculator (Warning Note & Restrictions Removed) ---
    if st.session_state.current_band == "KU-BAND":
        st.markdown("<div class='section-header'>3. KU-BAND LO CALCULATOR</div>", unsafe_allow_html=True)
        
        l1, l2, l3 = st.columns(3)
        # RF Input now works freely without range limits
        rf_in = l1.number_input("Input RF Uplink", value=st.session_state.up, format="%.3f", key="lo_rf_in")
        custom_lo = l2.number_input("Custom LO (e.g. 12800)", value=12800.0, format="%.1f")
    ...
    # --- Feature 4: Bandwidth & Span Calculator (Toggle & Restrictions Removed) ---
    st.markdown(f"<div class='section-header'>{sec_num}. BANDWIDTH & SPAN CALCULATOR</div>", unsafe_allow_html=True)
    
    # Toggle switch (st.radio) removed to allow completely free input
    # Default value uses current session state but does not restrict the user
    def_val = st.session_state.get('up', 6000.0)

    b1, b2, b3, b4 = st.columns(4)
    # Start and Stop fields work freely without any frequency or toggle restrictions
    start_f = b1.number_input("Start Frequency", value=def_val - 18.0, format="%.3f", key="calc_start_f")
    stop_f = b2.number_input("Stop Frequency", value=def_val + 18.0, format="%.3f", key="calc_stop_f")
